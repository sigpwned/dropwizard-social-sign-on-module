...
public class DefaultTwitterOAuth1HttpRequestAuthorizer
    implements TwitterOAuth1HttpRequestAuthorizer {
  public static final String DEFAULT_OAUTH_VERSION_VALUE = OAuth1.ONE_DOT_OH_OAUTH_VERSION_VALUE;

  public static final TwitterOAuth1HttpRequestSigner DEFAULT_SIGNER =
      HmacSha1TwitterOAuth1HttpRequestSigner.INSTANCE;
  ...
  public ModelHttpRequest authorize(ModelHttpRequest request, String consumerKey,
    ...
    parameters.add(Parameter.of(OAuth1.OAUTH_NONCE_NAME, nonce));
    parameters.add(Parameter.of(OAuth1.OAUTH_SIGNATURE_NAME, signatureString));
    parameters.add(
        Parameter.of(OAuth1.OAUTH_SIGNATURE_METHOD_NAME, getSigner().getOAuthSignatureMethod()));
    parameters.add(Parameter.of(OAuth1.OAUTH_TIMESTAMP, Long.toString(timestamp)));
    parameters.add(Parameter.of(OAuth1.OAUTH_VERSION_NAME, getOAuthVersion()));
    if (token != null)
    ...
    headers.addAll(request.getHeaders().getHeaders());
    headers.add(ModelHttpHeaders.Header.of(ModelHttpHeaderNames.AUTHORIZATION, authorization));

    return ModelHttpRequest.of(request.getVersion(), request.getMethod(), request.getUrl(),
        ModelHttpHeaders.of(headers), request.getEntity());
  }
