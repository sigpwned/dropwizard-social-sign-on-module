...
public class TwitterOAuth1HttpFilterTest {
  ...
  public void shouldSucceedIfEverythingGoesRight() throws Exception {
    ...
    server.enqueue(new MockResponse().setResponseCode(HttpURLConnection.HTTP_OK)
        .setBody(String.format("%s&%s", Parameter.of(OAuth1.OAUTH_TOKEN_NAME, oauthToken),
            Parameter.of(OAuth1.OAUTH_TOKEN_SECRET_NAME, oauthTokenSecret))));

    server.enqueue(new MockResponse().setResponseCode(HttpURLConnection.HTTP_OK)
        .setBody(String.format("%s&%s", Parameter.of(OAuth1.OAUTH_TOKEN_NAME, token),
            Parameter.of(OAuth1.OAUTH_TOKEN_SECRET_NAME, tokenSecret))));

    server.start();

    final TwitterOAuth1TokenStore store = mock(TwitterOAuth1TokenStore.class);
    when(store.getTwitterOAuth1TokenSecret(oauthToken)).thenReturn(Optional.of(oauthTokenSecret));
    ...
    when(handler.twitterOAuth1Authenticated(token, tokenSecret))
